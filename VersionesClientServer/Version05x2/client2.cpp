...
int main()
{
	...
	string menuInicial = "Log Out (O) - Message (N) - List (I) - Invitacion Tic Tac Toe (P) - Movimiento juego (W): ";
	...
	switch (type[0]){
		...
		case 'P':{
			...
			string pregJugar = "Jugamos TicTacToe: (Y/X) ";
		...
		case 'Y':{
			...
			string temp = "Y" + string(size_m) + string(msg) + string(userBuffSize) + string(receiver);
			//cout<<temp<<endl;
			write(SocketFD, temp.c_str(), temp.size()+1);
			break;
		}
		...
		case 'X':{
			...
			string pregJugar = "No quiero JUgar";
			for( int i =0; i < pregJugar.length(); i++){
				msg[i] = pregJugar[i];
			}

			string temp = "X" + string(size_m) + string(msg) + string(userBuffSize) + string(receiver);
			//cout<<temp<<endl;
			write(SocketFD, temp.c_str(), temp.size()+1);
			break;
		}
		...
		case 'W':{
			...
			printf("Colocar ficha en la posicion: ");
			...
			string temp = "W" +  string(size_m) + string(msg) + string(userBuffSize) + string(receiver);
			//cout<<temp<<endl;
			write(SocketFD, temp.c_str(), temp.size()+1);
			break;
			break;
...
void thread_read(int socketC) 
{
	...
	//Tic Tac Toe--------------------------
		...
		cout<<endl<<buffer<< " Si quiere jugar"<<endl;
	...
	// Rechazar el juego
	    ...
	    cout<<endl<<buffer<< " No quiere jugar"<<endl;
